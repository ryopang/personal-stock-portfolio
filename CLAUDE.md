...
**AI providers** — three providers (Gemini 3.8 Flash, Claude Sonnet 5, Claude Opus 5) unified behind the Vercel AI SDK (`streamText`). The registry lives in `src/lib/ai-providers.ts`; prompt text/builders in `src/lib/prompts.ts`. The provider is selectable in the UI per session. AI analysis is persisted to Redis; the chatbot is ephemeral and uses `useChat` from `@ai-sdk/react` (server responds via `toUIMessageStreamResponse`). Both AI routes are rate-limited per IP via `@upstash/ratelimit` (`src/lib/ratelimit.ts`).
