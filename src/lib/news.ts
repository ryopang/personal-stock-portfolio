...
// Yahoo Finance RSS fetching, parsing, relevance filtering, and translation.
// Shared by /api/news (paginated, translatable) and the AI analysis route
// (which injects raw articles into the prompt server-side).
...
/**
 * Fetch, dedupe, reclassify, relevance-filter, and date-sort news for a set
 * of Yahoo-format symbols. No translation or pagination — callers layer those.
 */
export async function fetchNewsForSymbols(rawSymbols: string[]): Promise<NewsItem[]> {
  const symbols = [...new Set(rawSymbols.filter(Boolean))].slice(0, 12);
...
export async function translateArticles(articles: NewsItem[], target: string): Promise<NewsItem[]> {
