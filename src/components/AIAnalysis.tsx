            ...
            {headerRows.length > 0 && (
              <thead>
                {headerRows.map((row, ri) => (
                  <tr key={ri} className="bg-surface-secondary border-b border-border">
                    {row.map((cell, ci) => (
                      <th key={ci} className="text-left py-2.5 px-4 font-semibold text-secondary whitespace-nowrap">
                        {inlineBold(cell)}
                      </th>
                    ))}
                  </tr>
