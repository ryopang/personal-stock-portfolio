...
// Desktop table row
export function HoldingTableRow({ holding, onEdit, onDelete }: Props) {
  ...
  return (
    ...
    >
      ...
      <td className="relative py-2 pl-16 pr-2">
        {holding.dailyChangePercent > 5 && (
          <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-base leading-none select-none" title="Up >5% today">🔥</span>
        )}
        {holding.dailyChangePercent < -5 && (
          <span className="absolute left-2.5 top-1/2 -translate-y-1/2 leading-none select-none" title="Down >5% today">
            <svg className="w-5 h-5 text-loss" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
      ...
      {/* 52-Week Range */}
      <td className="py-1.5 px-3 pr-6 text-center">
